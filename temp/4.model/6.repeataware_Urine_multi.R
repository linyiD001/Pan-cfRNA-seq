...
rm(list=ls())
setwd("/dssg/home/acct-dahan/share/cfRNA/RESULT/ANALYSIS/model/salmon_repeataware_stranded_allgenome/multi_N")

train_data<-read.csv("/dssg/home/acct-dahan/share/cfRNA/RESULT/ANALYSIS/model/salmon_repeataware_stranded_allgenome/train_data_N.csv",header=T,row.names = 1)
test_data<-read.csv("/dssg/home/acct-dahan/share/cfRNA/RESULT/ANALYSIS/model/salmon_repeataware_stranded_allgenome/test_data_N.csv",header=T,row.names = 1)
...
data_response$Patient<-sub(paste0("*", "_N"), "", rownames(data_response))
library(readxl)
...
library(Cairo)
CairoPDF("N_repeataware_scatter3D_plot.pdf", width = 7, height = 6,family='Arial')
#CairoPNG("N_repeataware_scatter3D_plot.png", width = 7, height = 6, units = "in", res = 300, family = "Arial")
scatter3D(data_merge$Prob_HC, data_merge$Prob_BS, data_merge$Prob_BC,
          col = col,
          colvar = NULL,   
          pch = 19,
          cex = 0.6,
...
ggsave("N_repeataware_Stage.png",height=8,width=12,dpi=300)
ggsave("N_repeataware_Stage.pdf",height=8,width=12,dpi=300)
...
setwd("/dssg/home/acct-dahan/share/cfRNA/RESULT/ANALYSIS/model/salmon_repeataware_stranded_allgenome/multi_N")
glmnet_model <- readRDS("/dssg/home/acct-dahan/share/cfRNA/RESULT/ANALYSIS/model/salmon_repeataware_stranded_allgenome/multi_N/glmnet_model.rds")
coefficients <- coef(glmnet_model, s = glmnet_model$lambda.min)
non_zero_vars <- list()

coefficients[[3]]
#for (class in names(coefficients)) {
coef_matrix <- coefficients[[3]]
coef_matrix<-as.matrix(coef_matrix)
coef_matrix<-as.data.frame(coef_matrix)
colnames(coef_matrix)<-'coef'
coef_matrix$name<-rownames(coef_matrix)
coef_matrix$coef<-as.numeric(coef_matrix$coef) 
non_zero_coeffs_1 <- coef_matrix[which(coef_matrix$coef>0),]
non_zero_coeffs_2 <- coef_matrix[which(coef_matrix$coef<0),]
data_list<-rbind(non_zero_coeffs_1,non_zero_coeffs_2)

write.csv(data_list ,'non_zero_coeffs.csv')
#glmnet_model$lambda.min
#cvfit$lambda.1se


##urine
coef<-read.csv("/dssg/home/acct-dahan/share/cfRNA/RESULT/ANALYSIS/model/salmon_repeataware_stranded_allgenome/multi_N/non_zero_coeffs.csv",header=T,row.names = 1)
DEG<-read.csv("/dssg/home/acct-dahan/share/cfRNA/RESULT/ANALYSIS/DEG_repeataware/salmon_repeataware_stranded_allgenome/N_type_C_vs_H_result.csv",header=T)
colnames(DEG)[1]<-'name'
DEG$name<-gsub("-", ".", DEG$name)
DEG$name<-gsub("\\(", "X.", DEG$name)
DEG$name<-gsub("\\)", ".", DEG$name)

data_merge<-merge(coef,DEG,by='name',all.x=T)

p <- ggplot(data_merge, aes(x =coef, y =log2FoldChange)) +
  labs(x=expression(bold('coef')), 
       y=(expression(bold('-Log'['2']*'(FoldChange)'))), 
       title=NULL) +
  geom_point(color='gray', alpha=1, size=2) +
  scale_color_manual(values = cols) +
  geom_point(data = subset(data_merge,log2FoldChange>1),
             aes(x =coef, y =log2FoldChange), fill="darkred", 
             size=1, color="darkred",shape=21)+
  geom_point(data = subset(data_merge,log2FoldChange<(-1)),
             aes(x =coef, y =log2FoldChange), fill="darkblue", 
             size=1, color="darkblue",shape=21) +
  scale_fill_manual(values = cols) +
  geom_text_repel(aes(label = name),
                  segment.alpha = 0.5, #segment.size = 0.5,
                  min.segment.length = 0.2, max.overlaps = 100,
  ...
  theme(axis.text=element_text(size=14),
        ...
        strip.text = element_text(size=14, face='bold'))+xlim(-0.2,0.2)
#theme(plot.margin =  margin(t = 0.25, r = 0.25, b = 0.25, l = 0.25, unit = "cm"))
p
ggsave("coeff_N_DEG_C_H.png",height=4,width=6,units='in',dpi=1200)
ggsave("coeff_N_DEG_C_H.pdf",height=4,width=6,units='in',dpi=1200)


coef<-read.csv("/dssg/home/acct-dahan/share/cfRNA/RESULT/ANALYSIS/model/salmon_repeataware_stranded_allgenome/multi_N/non_zero_coeffs.csv",header=T,row.names = 1)
DEG<-read.csv("/dssg/home/acct-dahan/share/cfRNA/RESULT/ANALYSIS/DEG_repeataware/salmon_repeataware_stranded_allgenome/N_type_C_vs_S_result.csv",header=T)
colnames(DEG)[1]<-'name'
DEG$name<-gsub("-", ".", DEG$name)
DEG$name<-gsub("\\(", "X.", DEG$name)
DEG$name<-gsub("\\)", ".", DEG$name)
...
ggsave("coeff_N_DEG_C_S.png",height=4,width=6,units='in',dpi=1200)
ggsave("coeff_N_DEG_C_S.pdf",height=4,width=6,units='in',dpi=1200)
