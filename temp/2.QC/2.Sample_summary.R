...
cfRNA_count<-read.csv("/dssg/home/acct-dahan/share/cfRNA/RESULT/MAPPING/salmon.repeataware/salmon.repeataware_stranded_allgenome/salmon.repeataware_simple_genename.csv",header=T,row.names = 1)
QC_list<-read.csv("/dssg/home/acct-dahan/share/cfRNA/RESULT/ANALYSIS/QC/QC_repeataware_list.csv",row.names = 1)
cfRNA_count<-cfRNA_count[,which(colnames(cfRNA_count) %in% QC_list[,1])]


tissue_count<-read.csv("/dssg/home/acct-dahan/share/tissue/RESULT/MAPPING/star/unanno_count.csv",header=T,row.names = 1)
...
length(which(clinic$Plasma=='plasma' & clinic$group=="C"))
length(which(clinic$Plasma=='plasma' & clinic$group=="R"))
length(which(clinic$Plasma=='plasma' & clinic$group=="S"))
length(which(clinic$Plasma=='plasma' & clinic$group=="H"))

length(which(clinic$Urine=='urine' & clinic$group=="C"))
length(which(clinic$Urine=='urine' & clinic$group=="R"))
length(which(clinic$Urine=='urine' & clinic$group=="S"))
length(which(clinic$Urine=='urine' & clinic$group=="H"))
...
length(which(clinic$Urine=='urine'& clinic$Plasma=='plasma' & clinic$group=="R"))
length(which(clinic$Urine=='urine'& clinic$Plasma=='plasma' & clinic$group=="S"))
length(which(clinic$Urine=='urine'& clinic$Plasma=='plasma' & clinic$group=="H"))
length(which(clinic$Urine=='urine'& clinic$Plasma=='plasma' & clinic$group=="C"))
