...
count_cfRNA<-read.csv("/dssg/home/acct-dahan/share/cfRNA/RESULT/MAPPING/salmon.standard/TPM_all_anno_by_gene_name.csv",header=T,row.names = 2)
count_cfRNA<-count_cfRNA[,-1]
GTEx_data<-read.csv("/dssg/home/acct-dahan/share/cfRNA/RESULT/ANALYSIS/cell_type_marker/database/GTEx_mean_TPM_anno_by_genename.csv",header=T,row.names=2)
GTEx_data<-GTEx_data[,-1]
count_cfRNA_select<-count_cfRNA[which(rownames(count_cfRNA) %in%rownames(GTEx_data)),]
GTEx_data_select<-GTEx_data[which(rownames(GTEx_data) %in%rownames(count_cfRNA)),]
GTEx_data_select <- GTEx_data_select[match(rownames(count_cfRNA_select), rownames(GTEx_data_select)), ]

all.equal(rownames(GTEx_data_select),rownames(count_cfRNA_select))

basis_matrix_rank_list<-apply(-GTEx_data_select,2,rank)
counts_rank_list<-as.data.frame(apply(-count_cfRNA_select,2,rank))


cor_table<-matrix(nrow=54,ncol=222)
...
#write.csv(cor_table,"spearman_cor_between_cf_GTEx.csv")
...
library(Cairo)
CairoPDF(file = "heatmap_N_group_GTEx.pdf", width = 20, height = 10,family='Arial')
#CairoPNG(file = "heatmap_N_group_GTEx.png", width = 20, height = 10,res=600,units='in',family='Arial')

# Plot the heatmap with the ordered data
pheatmap(cor_table_ordered, 
         cluster_rows = TRUE, 
         show_rownames = TRUE, 
