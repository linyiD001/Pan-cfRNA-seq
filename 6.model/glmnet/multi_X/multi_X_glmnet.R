...
rm(list=ls())
set.seed(20251012)
setwd("/dssg/home/acct-dahan/share/BC/cfRNA/RESULT/ANALYSIS/model/star_standard/glmnet/multi_X")
...
for (i in 1:100)
{
  fit <- cv.glmnet(train_x, train_y, family = "multinomial",type.measure = "class",type.multinomial = "grouped",nfolds=10)
...
save.image("multi_X.RData")

# Predict the classes for training and test datasets
train_predictions <- predict(fit, newx = train_x,s=bestlambda,type = "class")
test_predictions <- predict(fit, newx = test_x,s=bestlambda,type = "class")
...
library(plot3D)
library(Cairo)
CairoPDF("X_standard_scatter3D_plot.pdf", width = 7, height = 6,family='Arial')
#CairoPNG("X_standard_scatter3D_plot.png", width = 7, height = 6, units = "in", res = 300, family = "Arial")
scatter3D(x, y, z,
          col = col,
          colvar = NULL, 
          pch = 19,
...
p2
ggsave("X_standard_Stage.png",height=6,width=8,dpi=300)
ggsave("X_standard_Stage.pdf",height=6,width=8,dpi=300)
