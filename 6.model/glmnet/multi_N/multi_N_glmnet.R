...
rm(list=ls())
set.seed(20251012)
setwd("/dssg/home/acct-dahan/share/BC/cfRNA/RESULT/ANALYSIS/model/star_standard/glmnet/multi_N")
...
fit <- glmnet(train_x, train_y, family = "multinomial",type.measure = "class", type.multinomial = "grouped")
...
for (i in 1:100)
{
  cvfit <- cv.glmnet(train_x, train_y, family = "multinomial",type.measure = "class",type.multinomial = "grouped",nfolds=10)
...
fit <- glmnet(train_x, train_y, family = "multinomial",type.measure = "class",type.multinomial = "grouped",lambda=bestlambda)
coef(fit)

coef_i <- as.data.frame(coef(fit))
coef_i 
nonzero_genes <- rownames(coef_i)[rowSums(coef_i != 0) > 0]
is.na(nonzero_genes)
nonzero_genes <- setdiff(nonzero_genes, "(Intercept)")
nonzero_genes
...
save.image("multi_N.RData")

#fit<- readRDS("/dssg/home/acct-dahan/share/BC/cfRNA/RESULT/ANALYSIS/model/star_standard/multi_N/glmnet_model.rds")

# Predict the classes for training and test datasets
train_predictions <- predict(fit,s=bestlambda,newx = train_x, type = "class")
test_predictions <- predict(fit,,s=bestlambda,newx = test_x,type = "class")
...
ggsave("train_confusion_matrix_multiclass.png", train_cm_plot, width = 6.5, height = 6, units = "in", dpi = 300)
ggsave("train_confusion_matrix_multiclass.pdf", train_cm_plot, width = 6.5, height = 6, units = "in", dpi = 300)

ggsave("test_confusion_matrix_multiclass.png", test_cm_plot, width = 6.5, height = 6, units = "in", dpi = 300)
ggsave("test_confusion_matrix_multiclass.pdf", test_cm_plot, width = 6.5, height = 6, units = "in", dpi = 300)

# Predict the classes for training and test datasets
train_response <- predict(fit, newx = train_x,type = "response")
train_response<-as.data.frame(train_response)
test_response<- predict(fit, newx = test_x,type = "response")
...
plot_ly(data_merge, 
        ...
        z = ~Prob_BC, 
        color = ~group, 
        colvar = NULL,   # <- 关键：使用 col 作为点颜色（而不是作为 palette）
...
library(plot3D)
library(Cairo)
CairoPDF("N_standard_scatter3D_plot.pdf", width = 7, height = 6,family='Arial')
#CairoPNG("N_standard_scatter3D_plot.png", width = 7, height = 6, units = "in", res = 300, family = "Arial")
scatter3D(x, y, z,
          col = col,
          colvar = NULL, 
          pch = 19,
...
legend("topright",                   # 图例位置，也可以用坐标，如 c(1,1)
       legend = names(group_colors), # 标签名字
       col = group_colors,            # 颜色
       pch = 19,                     # 点形状
       cex = 1,                      # 大小
       bty = "n")                    # 不画图例框
...
p2<-ggplot(data_merge, aes(x = StageT, y = Prob_BC, fill = StageT)) +
  ...
  theme(
    ...
    #axis.ticks.x = element_blank(),
    axis.title = element_text(size = 15,face="bold",margin = margin(t = 5)),
    axis.title.y = element_text(size = 15,face="bold",margin = margin(r = 0)),
  ...
  theme(legend.position = "right")# 修改字号和居中
p2

ggsave("N_standard_Stage.png",height=6,width=8,dpi=300)
ggsave("N_standard_Stage.pdf",height=6,width=8,dpi=300)
