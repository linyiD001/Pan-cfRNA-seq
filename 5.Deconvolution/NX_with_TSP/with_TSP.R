library(readxl)
setwd("/dssg/home/acct-dahan/share/BC/cfRNA/RESULT/ANALYSIS/cell_type_marker/NX_with_TSP")

count_cfRNA<-read.csv("/dssg/home/acct-dahan/share/BC/cfRNA/RESULT/ANALYSIS/DEG_standardstar/star_standard_anno_count.csv",header=T,row.names = 2)
count_cfRNA<-count_cfRNA[,-1]
TSP_data<-read.table("/dssg/home/acct-dahan/share/BC/cfRNA/RESULT/ANALYSIS/cell_type_marker/database/tsp_v1_basisMatrix.txt",header=T,sep='\t',row.names=1)
...
cor_table<-matrix(nrow=62,ncol=223)
...
#pdf("heatmap_N_TSP_cor.pdf", width = 20, height = 10)
png("heatmap_N_TSP_cor.png", width = 20, height = 10,unit='in',res = 300)
pheatmap(cor_table_plot, 
         cluster_rows=T, 
         show_rownames=T,
         cluster_cols=T,
         show_colnames = F, 
